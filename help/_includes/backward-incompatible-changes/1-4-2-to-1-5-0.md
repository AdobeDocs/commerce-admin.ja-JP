...
---
# 1.4.2と1.5.0の間のB2B後方互換性のない変更

## 1.4.2-1.5.0

### ハイライト {#b2b-BICs-142-150-highlights}

複数の会社の割り当てが追加されたことで、会社のユーザーアカウントは複数の`company_id`値を持つことができるようになりました。 `Magento\Company\Api\Data\CompanyCustomerInterface`が更新され、ユーザーのデフォルト `company_id`が設定されました。 デフォルトは、会社のユーザーアカウントに割り当てられた最初の会社に設定されます。

以前のリリースからアップグレードする場合は、Adobeでは、`Magento\Company\Api\Data\CompanyCustomerInterface`を使用するクラスに次のメソッドを実装することをお勧めします。
