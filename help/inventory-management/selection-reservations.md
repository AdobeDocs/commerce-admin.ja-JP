...
SSAは、在庫とソースを使用して、入荷した製品リクエストの販売チャネルをチェックし、利用可能な在庫を決定します。
...
このアルゴリズムは、注文を処理し、配送するのに役立ちます。 アルゴリズムを実行して、製品を出荷するための最適なソースに関する推奨事項を取得したり、選択内容を上書きしたりして、次のことをおこなうことができます。
...
カスタム在庫のソースを設定、割り当て、注文するには、[在庫のソースの優先順位付け](stocks-prioritize-sources.md)を参照してください。
...
距離優先アルゴリズムは、配送先住所の場所と発送先住所を比較して、出荷を処理するための最も近い発送元を決定します。 距離は、読み込まれたデータベースの場所またはGoogleの方向（運転、ウォーキング、または自転車）を使用して、ある場所から別の場所へ移動するのに費やした物理的な距離または時間によって決まります。
...
- **未処理の予約**。 システムは、補償されていない最初の予約をすべて集計します。 この数字は常に負です。 お客様Aが10件の予約を行い、お客様Bが5件の予約を行った場合、製品の合計–15件の未予約が発生します。

そのため、顧客が40 （55 + -15）ユニット未満の注文をした場合、加盟店は受注に対応できます。
...
メタデータ `object_type`は`order`である必要があり、`object_id`は注文IDです。
...
1. 顧客は20品目の請求書を送信し、注文した5つのユニットをキャンセルします。
...
- *発送済み注文* – 注文が（全部または一部）発送されると、補償予約が入力され、その金額がクリアされます。 例えば、2つのバックパックを配送すると、そのSKUと在庫の+2の予約が入り、保留が解除されます。 製品数量は出荷のために2によって直接減少します。 計算された販売可能数量も、削減された在庫量に対して更新されますが、予約の影響は受けません。
