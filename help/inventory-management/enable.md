...
description: ' [!DNL Inventory Management] を有効または無効にし、店舗または商品レベルで在庫を管理して、販売可能な数量とフルフィルメントの追跡を制御します。'
...
[!DNL Inventory Management]が有効になっている場合、販売活動で在庫が更新されます：

- 販売可能な数量は、顧客が商品をカートに入れ、チェックアウトを完了し、注文を発送または返金した場合、在庫ごとに更新されます。
- 在庫量を更新すると、ソースでの新規または譲渡された在庫がオンライン販売で利用できるようになります。
- バックオーダーは、追加の設定なしで設定されたしきい値を尊重します。
- アルゴリズムによるレコメンデーションまたは手動でのソース選択を使用して、1つ以上のソースから部分的または完全な出荷を作成できます。
