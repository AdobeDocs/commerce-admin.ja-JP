...
| [!UICONTROL Number] [!BADGE SaaSのみ]{type=Positive url="https://experienceleague.adobe.com/ja/docs/commerce/user-guides/product-solutions" tooltip="Adobe Commerce as a Cloud ServiceおよびAdobe Commerce Optimizer プロジェクト（Adobeが管理するSaaS インフラストラクチャ）にのみ適用されます。"} | 10進数値を格納する数値入力フィールド。 **Price**&#x200B;入力タイプとは異なり、通貨の書式設定は適用されず、負の値を受け入れます。 この入力タイプは、測定、寸法、または温度範囲などの技術仕様に使用します。 |
