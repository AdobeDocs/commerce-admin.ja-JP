...
MAP 機能により、専用のが追加されます _クリックして価格_ 通常の製品価格ではなくリンク。 店舗の価格が商品の最低設定価格を下回っている場合、店舗側で価格情報を処理する方法は 2 つあります。 1 つ目の方法は、価格が表示されないことです。 購入者が _クリックして価格_ ボタンをクリックすると、商品を販売している実際の価格が表示されます。 2 つ目の方法は、リスト/市場価格が取り消し線で表示され、価格が低いことを強調することです。
...
| 製品タイプ | 説明 |
|--- |--- |
| [シンプル](product-create-simple.md), [仮想](product-create-virtual.md) | 実際の価格は、カタログリストや製品ページに自動的には表示されませんが、に従ってのみ含まれます [!UICONTROL Display Actual Price] の設定値。 カスタムオプションの価格は通常どおり表示されます。 |
| [グループ化](product-create-grouped.md) | 関連するシンプルな製品の価格は、カタログリストや製品ページに自動的には表示されませんが、に従ってのみ含まれます [!UICONTROL Display Actual Price] の設定値。 |
| [設定可能](product-create-configurable.md) | 実際の価格は、カタログリストや製品ページに自動的には表示されませんが、に従ってのみ含まれます [!UICONTROL Display Actual Price] の設定値。 オプション価格は通常どおりに表示されます。 |
| [バンドル](product-create-bundle.md) （固定価格あり） | 実際の価格はカタログページに自動的には表示されませんが、に従ってのみ含まれます [!UICONTROL Display Actual Price] の設定値。 バンドル品目の価格は通常どおり表示されます。 動的価格設定のバンドル製品には、MAP は使用できません。 |
| [ダウンロード可能](product-create-downloadable.md) | 実際の価格は、カタログリストや製品ページに自動的には表示されませんが、に従ってのみ含まれます [!UICONTROL Display Actual Price] の設定値。 各ダウンロードリンクに関連付けられている価格は、通常どおりに表示されます。 |
...
1. 該当する場合、を設定します **[!UICONTROL Store View]** をクリックし、設定が適用されるビューに移動します。

1. 左側のパネルで、を展開します **[!UICONTROL Sales]** を選択します **[!UICONTROL Sales]** その下に。

1. を展開 ![展開セレクター](../assets/icon-display-expand.png) この _[!UICONTROL Minimum Advertised Price]_セクション。

1. 必要に応じて、を設定します **マップを有効にする** 対象： `Yes`.

   ![マップ設定](./assets/sales-minimum-advertised-price.png){width="600" zoomable="yes"}

   これらの設定オプションの詳細なリストについては、を参照してください [_広告の最低価格_](../configuration-reference/sales/sales.md#minimum-advertised-price) が含まれる _設定リファレンス_.
...
1. 実際の価格を顧客に表示するタイミングと場所を決定するには、次の操作を行います。
   ...
   - を設定 **実際の価格を表示** を次のいずれかに変更します。
      - `In Cart`
      - `Before Order Confirmation`
      - `On Gesture (on click)`

1. に表示するテキストを入力 **[!UICONTROL Default Popup Text Message]**.

1. に表示する追加の説明を入力してください **[!UICONTROL Default "What's This" Text Message]**.

1. 完了したら、 **[!UICONTROL Save Config]**.

#### 方法 2：単一の製品の MAP の設定

1. 日 _Admin_ サイドバー、に移動 **[!UICONTROL Catalog]** > **[!UICONTROL Inventory]** > **[!UICONTROL Products]**.
...
1. 左側のパネルで、を展開します **[!UICONTROL Advanced Settings]** を選択します **[!UICONTROL Advanced Pricing]**.
   ...
   >[!NOTE]
   >
   >この [!UICONTROL Manufacturer's Suggested Retail Price] および [!UICONTROL Display Actual Price] フィールドが表示されるのは、 [広告の最低価格](../configuration-reference/sales/sales.md#minimum-advertised-price) 設定で有効になっています。

1. を入力 **[!UICONTROL Manufacturer's Suggested Retail Price]** （MSRP）。

   この例では、製品価格は$54.00 で、MSRP は 59.95 です。

   ![メーカー希望小売価格](./assets/product-price-msrp.png){width="600" zoomable="yes"}

1. を設定 **[!UICONTROL Display Actual Price]** を次のいずれかに変更します。

   - `Use config` - （デフォルト）表示設定を次のように適用します [設定済み](../configuration-reference/sales/sales.md#minimum-advertised-price) ストア用。 |
   - `On Gesture`  – 顧客が商品の実際の価格を _クリックして価格_ または _これは何ですか。_ リンク。
   - `In Cart`  – 実際の商品価格が買い物かごに表示されます。
