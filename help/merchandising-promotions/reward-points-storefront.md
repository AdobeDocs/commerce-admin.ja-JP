---
title: ポイントストアフロント体験への報酬
description: 顧客がストアフロントアカウントで報酬ポイント残高の履歴にアクセスする方法について説明します。
exl-id: 6aa2d406-95c1-408c-a971-5c2a4132a0c8
feature: Rewards, Promotions/Events, Customers, Storefront
TQID: https://experienceleague.adobe.com/R2LBruTKbg4EhfNJ4tO795Y4LPS094UtRpZIuVNFe6c
product_v2:
  - id: eadea719-cf89-469b-a6fd-a236a7138047
feature_v2:
  - id: d1e21356-0064-4f48-9089-16e3f0dbd2a6
role_v2:
  - id: b69b2659-1057-424e-8fc5-ed9e016dc554
  - id: c66ffd68-0f65-42bb-aa23-b4020f12e0bd
  - id: f8a45b24-4be7-4f1b-909b-60d06b483a20
level_v2:
  - id: b5a62a22-46f7-4f0d-b151-3fc640bef588
  - id: e8ccd51f-da0d-4e3b-939b-e30d5ebb1ea5
topic_v2:
  - id: b5520579-b31f-4df7-9281-f0d9f91e2edc
  - id: eddd9b14-83bd-4ff4-9072-54a4a484abb7
source-git-commit: b9626700040bdf9de5aa9a987dec28a08243a9e1
workflow-type: tm+mt
source-wordcount: 149
ht-degree: 0%

---

# ポイントストアフロント体験への報酬

{{ee-feature}}

顧客アカウントの[報酬ポイント &#x200B;](rewards-loyalty.md) セクションには、顧客が獲得した報酬ポイントの現在の残高と、その報酬ポイント残高の履歴が表示されます。

![&#x200B; ポイントの付与](./assets/account-dashboard-reward-points.png){width="700" zoomable="yes"}

## チェックアウト時に報酬ポイントを交換する

`Points to Currency`方向の[&#x200B; ポイント換算レート &#x200B;](reward-exchange-rates.md)が設定されている場合、お客様はチェックアウト時にポイントを使用できます。

1. 顧客は、必要な商品をすべてショッピングカートに入れた後、チェックアウトに移動します。

1. 必要なすべての出荷情報を入力し、_確認と支払い_ ステップに移動します。

1. 「_[!UICONTROL Reward points]_」セクションで、利用可能なポイント数とその通貨値を確認します。

1. **[!UICONTROL Use reward points]**&#x200B;をクリックします。

![&#x200B; チェックアウト時のポイントへの報酬](./assets/reward-points-on-checkout.png){width="700" zoomable="yes"}

利用可能なポイントの割引は、小計に適用されます。

>[!NOTE]
>
>ご利用いただける残高が注文の合計金額を超える場合は、別のお支払い方法は必要ありません。
