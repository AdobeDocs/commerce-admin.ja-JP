...
>[!NOTE]
>
>複数またはすべての製品の URL 書き換えを同時に更新するには、を参照してください。 [複数の URL の書き換え](url-rewrite-product.md#multiple-url-rewrites).

用語 _書換え_ および _リダイレクト_ 多くの場合、同じ意味で使用されますが、少し異なるプロセスを指します。 URL の書き換えにより、ブラウザーでの URL の表示方法が変更されます。 URL リダイレクトは、サーバーに保存された URL を更新します。 URL リダイレクトは、一時的または永続的に設定できます。 製品、カテゴリまたはページの URL キーを簡単に変更し、既存のリンクを保持できるように、ストアでは URL の書き換えとリダイレクトを使用しています。

デフォルトでは [自動 URL リダイレクト](url-redirect-product-automatic.md) ストアとに対して有効になっています **古い URL の永続的なリダイレクトの作成** 各製品の「URL キー」フィールドの下で、チェックボックスが選択されています。
...
正規ページを指定するには、を参照してください [Google Search Central ドキュメント](https://developers.google.com/search/docs/crawling-indexing/consolidate-duplicate-urls).
...
1. 左側のパネルで、 **[!UICONTROL General]** 展開済み、を選択 **[!UICONTROL Web]**.
...
1. を設定 **[!UICONTROL Use Web Server Rewrites]** ご希望に合わせてください。
