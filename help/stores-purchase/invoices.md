...
1. 受注のヘッダーで、「**[!UICONTROL Invoice]**」オプションを選択します。
   ...
   >[!NOTE]
   >
   >特定の _[!UICONTROL Invoice]_支払方法 [ の ](../configuration-reference/sales/payment-methods.md#payment-actions) 支払アクション [ が ](../configuration-reference/sales/payment-methods.md) に設定され、請求書が自動生成される場合、`Authorize and Capture` オプションは表示されません。 これは、注文が行われ、支払い方法の支払いアクションが `Authorize` に設定され、注文が請求された場合にも当てはまります。
...
1. _[!UICONTROL Invoices]_グリッドで請求書を見つけ、**[!UICONTROL View]**Action_ 列の _をクリックします。
