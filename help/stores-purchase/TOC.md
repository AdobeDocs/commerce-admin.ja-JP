...
# 実店舗および購入エクスペリエンスガイド {#stores-sales}

+ [実店舗および購入エクスペリエンスガイド](guide-overview.md)
+ [ストア体験と購入体験の概要](introduction.md)
+ サイトとストアの管理 {#site-store}
   + [ストアメニュー](stores-menu.md)
   + [ストア構造とサイト構造](stores.md)
   + [ストアビュー](store-views.md)
   + [店舗のローカライズ](store-localize.md)
   + [URLの保存](store-urls.md)
   + 税金 {#taxes}
      + [概要](taxes.md)
      + [税金設定](tax-settings-general.md)
      + [価格表示の設定](display-settings.md)
      + [税ルール](tax-rules.md)
      + [税区分](tax-class.md)
      + [固定製品税](fixed-product-tax.md)
      + [非表示の税計算](hidden-tax-calculation.md)
      + [税区及び税率](tax-zones-rates.md)
      + [付加価値税（VAT）](vat.md)
      + [各国の租税ガイドライン](international-tax-guidelines.md)
   + 通貨 {#currency}
      + [概要](currency.md)
      + [通貨設定](currency-configuration.md)
      + [通貨レートの更新](currency-update.md)
   + [セールスメール](sales-email.md)
   + [販売ドキュメント](sales-documents.md)
+ POS {#point-of-purchase}
   + [今すぐ購入](checkout-instant-purchase.md)
   + 買い物かご {#cart}
      + [概要](cart.md)
      + [カート設定](cart-configuration.md)
      + [カートの永続性](cart-persistent.md)
      + [SKUで注文](order-by-sku.md)
   + ショッピング支援 {#assist}
      + [ショッピングカートの管理](shopping-assisted-cart-manage.md)
      + [注文の作成](customer-account-create-order.md)
      + [顧客注文の更新](order-update.md)
   + チェックアウト {#checkout}
      + [概要](checkout-process.md)
      + [1 ページのチェックアウト](checkout-one-page.md)
      + [ゲストチェックアウト](checkout-guest.md)
   ...
   + ギフトカード {#gift-cards}
      ...
      + [ギフトカードアカウント](product-gift-card-accounts.md)
+ 買い物客ツール {#shopper-tools}
   + [友達にメール](email-a-friend.md)
   + ウィッシュリスト {#wish-lists}
      + [概要](wishlists.md)
      + [ウィッシュリストの設定](wishlist-configuration.md)
      + [ストアフロント体験を求める](wishlist-storefront.md)
   + [製品の比較](product-compare.md)
   + [最近閲覧または比較](products-viewed-compared.md)
   + [再注文を許可](reorders-allow.md)
   + [キャンセル注文を許可](cancel-allow.md)
+ 支払い {#payments}
   + [概要](payments.md)
   + PayPal決済ソリューション {#paypal}
      ...
      + [PayPalの契約形態](paypal-billing-agreements.md)
      + [PayPal決済レポート](paypal-settlement-reports.md)
   + [Braintree](braintree.md)
   + [保存されている支払い方法](stored-payment-methods.md)
   + オフラインのお支払い方法 {#offline}
      + [小切手と為替](check-money-order.md)
      + [代金引換](cash-on-delivery.md)
      + [銀行振込](bank-transfer.md)
      + [発注](purchase-order.md)
      + [小計チェックアウトをゼロにする](zero-subtotal-checkout.md)
+ 注文フローの管理 {#order-management}
   ...
   + 注文 {#orders}
      ...
      + [スケジュールされた注文操作](order-scheduled-operations.md)
   ...
   + 返品 {#returns}
      + [概要](returns.md)
      + [返品の設定](rma-configure.md)
      + [属性を返す](attributes-returns.md)
      + [ストアフロント体験を返す](rma-customer-experience.md)
   + [取引](transactions.md)
+ 配信 {#delivery}
   + [概要](delivery.md)
   ...
   + 基本的な配信方法 {#basic-methods}
      ...
      + [実店舗での配送](shipping-in-store-delivery.md)
   + 運送業者 {#shipping-carriers}
      + [配送業者の設定](carriers.md)
      + [UPS](ups.md)
      + [USPS](usps.md)
      + [FedEx](fedex.md)
