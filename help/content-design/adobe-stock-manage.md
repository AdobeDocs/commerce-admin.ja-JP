...
[Adobe Stock](https://stock.adobe.com)の画像は、独自の画像コンテンツをアップロードする代わりに使用できます。 一般的なユースケースのひとつは、ページ作成時に画像コンテンツをアップロードして配置することです。

[[!DNL Media Gallery]](media-gallery.md)は、Adobe Stockとの直接的な統合を提供し、ギャラリーページから画像を直接ライセンス供与することが容易になります。
...
**_Adobe Stock アセットを検索し、ストック画像をカテゴリに追加するには:_**
...
1. 左上の&#x200B;**[!UICONTROL Search by keyword]**&#x200B;入力フィールドにキーワード検索を入力し、拡大鏡をクリックするか、**Enter**&#x200B;を押します。
   ...
   ![Adobe Stockの「マンゴー」キーワードの検索結果](./assets/adobe-stock-search-keyword.png){width="600" zoomable="yes"}

### Adobe Stock アセットのフィルタリング

1. [Adobe Stock アセットのキーワード検索を実行](#search-for-adobe-stock-assets-by-keywords)。

1. **[!UICONTROL Filters]**&#x200B;をクリックします。

   検索結果の絞り込みに使用できるフィルターはいくつかあります。

   | フィルター | 説明 |
   |---|---|
   | [!UICONTROL Subcategory] | **写真**&#x200B;または&#x200B;**イラスト**&#x200B;の画像のフィルター |
   | [!UICONTROL Orientation] | サイズ、形状、アスペクトによる画像のフィルタリング |
   | [!UICONTROL Color] | カラーパレットを使用して、画像をカラーでフィルタリングする |
   | [!UICONTROL Price] | コストに基づいて画像をフィルタリング |
   | [!UICONTROL Safe search] | セーフ検索を有効または無効にする |
   | [!UICONTROL Isolated Assets] | 表示を単色の背景に被写体が表示される&#x200B;_孤立アセット_&#x200B;のみに制限します |
...
1. 検索結果で画像をクリックします。
   ...
   - 画像の大きなバージョン
   - _[!UICONTROL Dimensions]_、_[!UICONTROL File type]_、_[!UICONTROL Category]_、_[!UICONTROL File]_、_キーワード_&#x200B;などの画像メタデータ
   - 関連する画像（同じ&#x200B;_シリーズ_&#x200B;または&#x200B;_モデル_&#x200B;の画像など）
   - [[!UICONTROL Save Preview]](adobe-stock-save-preview.md)や[[!UICONTROL Save (and optionally license) Image]](adobe-stock-license-image.md)などのアクション ボタン
...
1. **[!UICONTROL Save Preview]**&#x200B;をクリックします。
   ...
   このアクションは、画像をメディアストレージに保存するために使用するファイル名を指定するためのプロンプトを表示します。 デフォルトのファイル名が指定されますが、好みに合わせて名前をカスタマイズできます。
