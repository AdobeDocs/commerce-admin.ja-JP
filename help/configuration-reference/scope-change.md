...
多くの設定設定の右側にある&#x200B;_[!UICONTROL Use System Value]_チェックボックスは、現在の設定スコープ内のデフォルトのフィールド値を適用または上書きするために使用されます。 チェックボックスを選択すると、デフォルトのフィールド値を変更できません。 値を変更するには、チェックボックスをオフにして新しい値を入力します。 システム値を変更するたびに確認を求められます。
