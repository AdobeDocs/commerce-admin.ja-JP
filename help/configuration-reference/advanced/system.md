...
description: の設定を確認します。 [!UICONTROL Advanced] &gt; [!UICONTROL System] コマース管理者のページ。
...
これらの構成設定の変更の詳細は、次を参照してください [Cron （スケジュールされたタスク）](../../systems/cron.md).
...
| フィールド | [範囲](../../getting-started/websites-stores-views.md#scope-settings) | 説明 |
|--- |--- |--- |
| [!UICONTROL Successful Messages Lifetime] | グローバル | 成功したメッセージの有効期間を分単位で決定します。 クリーンアップをスキップするには、ゼロを入力します。 デフォルト： `10080` （7 日間） |
| [!UICONTROL New Messages Lifetime] | グローバル | 新しいメッセージの有効期間を分単位で指定します。 クリーンアップをスキップするには、ゼロを入力します。 デフォルト： `10080` （7 日間） |
| [!UICONTROL Failed Messages Lifetime] | グローバル | 失敗したメッセージの有効期間を分単位で決定します。 クリーンアップをスキップするには、ゼロを入力します。 デフォルト： `10080` （7 日間） |
...
これらの設定の変更の詳細については、を参照してください [メール通信を設定する](../../systems/email-communications.md) が含まれる _管理システムガイド_.

>[!IMPORTANT]
>
>**セキュリティに関する通知** 最近特定された潜在的なリモートコード実行の悪用から保護するために、すべてのマーチャントがメール送信設定を直ちに設定することをお勧めします。 この問題が解決されるまで、を使用しないことを強くお勧めします [!DNL Sendmail] メール通信の場合。 が含まれる [!UICONTROL Mail Sending Settings]必ずしてください [!UICONTROL Set Return Path] はに設定されています。 `No`.
...
この設定の変更の詳細については、を参照してください。 [通貨の設定](../../stores-purchase/currency-configuration.md) が含まれる _店舗および購入エクスペリエンスガイド_.
...
これらの設定の変更の詳細については、を参照してください [セッション管理](../../systems/security-session-management.md) が含まれる _管理システムガイド_.

| フィールド | [範囲](../../getting-started/websites-stores-views.md#scope-settings) | 説明 |
|--- |--- |--- |
| [!UICONTROL Max Session Size in Admin] | グローバル | セッションの最大サイズをバイト単位で制限します。 使用方法 `0` を無効にします。 |
| [!UICONTROL Max Session Size in Storefront] | グローバル | セッションの最大サイズをバイト単位で制限します。 使用方法 `0` を無効にします。 |
...
これらの設定の変更の詳細については、を参照してください [システム通知](../../systems/notifications.md) が含まれる _管理システムガイド_.
...
これらの設定の変更の詳細については、を参照してください [システムバックアップ](../../systems/backups.md) が含まれる _管理システムガイド_.
...
![詳細設定 – 管理者アクションのログのアーカイブ](./assets/system-admin-actions-log-archiving.png)<!-- zoom -->

これらの設定の変更の詳細については、を参照してください [アクションログアーカイブ](../../systems/action-log-archive.md) が含まれる _管理システムガイド_.
...
これらの設定の変更の詳細については、を参照してください [フルページキャッシュ](../../systems/cache-management.md#full-page-caching) が含まれる _管理システムガイド_.
...
| [!UICONTROL Handles param size] | global | の最大数を指定します [レイアウトハンドル](https://developer.adobe.com/commerce/frontend-core/guide/layouts/#layout-handles) でを処理します [`{BASE-URL}/page_cache/block/esi`](https://experienceleague.adobe.com/docs/commerce-operations/configuration-guide/cache/use-varnish-esi.html) HTTP エンドポイント。 サイズを制限すると、セキュリティとパフォーマンスが向上する可能性があります。 デフォルト値 `100` |
...
| [!UICONTROL Export VCL for Varnish 4] | グローバル | をエクスポートします `varnish.vcl` バージョン 4 のファイル。 |
| [!UICONTROL Export VCL for Varnish 5] | グローバル | をエクスポートします `varnish.vcl` バージョン 5 のファイル。 |
| [!UICONTROL Export VCL for Varnish 6] | グローバル | をエクスポートします `varnish.vcl` ファイル（バージョン 6 用）。 |
...
これらの設定の変更の詳細については、を参照してください [メディアデータベースの使用](../../content-design/media-storage-database.md) が含まれる _コンテンツおよびデザインガイド_.
...
これらの設定の変更の詳細については、を参照してください [一括アクション](../../systems/action-log-bulk-actions.md) が含まれる _管理システムガイド_.

| フィールド | [範囲](../../getting-started/websites-stores-views.md#scope-settings) | 説明 |
|--- |--- |--- |
| [!UICONTROL Days Saved in Log] | グローバル | で一括アクションを保持する日数を決定します _一括アクションログ_ アーカイブ。 デフォルト： `60` |
...
これらの設定の変更の詳細については、を参照してください [スケジュールされた読み込みと書き出し](../../systems/data-scheduled-import-export.md) が含まれる _管理システムガイド_.
...
| [!UICONTROL Error Email Template] | グローバル | インポート/エクスポート ファイルのクリーニング エラー通知に使用する電子メール テンプレートを識別します。 デフォルトのテンプレート： `File History Clean Failed` |
...
これらの設定の詳細については、を参照してください。 [Adobe Stockの統合](../../content-design/adobe-stock.md) が含まれる _コンテンツおよびデザインガイド_.
