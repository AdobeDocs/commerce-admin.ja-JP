---
title: 会社アカウントの管理
description: 会社ページ、およびグリッドで使用できるツールで、Adobe Commerceストアの会社アカウントを管理するのに役立つ情報を示します。
...
以下を使用します。 _[!UICONTROL Search]_～で会社を探す機能を持つ&#x200B;**会社**キーワード別のグリッド。 この場合、**会社名**および&#x200B;**親**列。 次の条件でフィルターできます。**会社タイプ**親会社とその関連会社を表示するか、子会社のみを表示するかを選択します。
...
1. を設定します。 **[!UICONTROL Actions]** ～を制御する `Set Active`.
...
1. を設定します。 [!UICONTROL **アクション**] コントロールを次のいずれかに設定します。
...
1. Adobe Analytics の _アクション_ 列の右端にある「 [!UICONTROL **編集**].
...
1. を設定します。 **[!UICONTROL Actions]** ～を制御する `Block`.
...
1. を設定します。 **[!UICONTROL Actions]** ～を制御する `Convert Currency`.
...
| [!UICONTROL Reason for Change] | 会社がクレジット制限を超えることを許可または禁止する理由を説明するメモ。 このフィールドは、クレジット制限を超える権限が変更された場合にのみ有効です。 |
