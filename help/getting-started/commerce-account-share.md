...
「請求履歴」セクションには、請求システムの変更前に作成された古い請求書のみが表示されます。 新しい請求書がリストに表示されない場合、それらの請求書は新しいシステムに移行されており、このビューからアクセスできません。
...
1. 開始する前に、[!DNL Commerce] 新しい共有アクセス権限 **の** アカウントから次の情報を取得します。
...
1. [!UICONTROL _New User Information]_で次の操作を行います。
   ...
   - 新しいユーザーの **[!UICONTROL Account ID]** アカウントの [!DNL Commerce] を入力します。
