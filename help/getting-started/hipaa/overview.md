...
- **[カタログのエンリッチメント](../../catalog/catalog-enrichment.md)** – この機能は、HIPAA対応の境界外のストアフロントおよびカタログ シグナルを分析する非HIPAA対応AI サービス（[!DNL Commerce Catalog Agent]および[!DNL Adobe LLM Optimizer]）に依存しているため、無効になっています。 この機能を有効にすると、AdobeのHIPAA対応サービスの対象外のシステムにPHIを公開する可能性があります。
